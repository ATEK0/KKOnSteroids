    ...
    <SideNavBar>
        <template v-slot>
            ...
            <div class="d-flex justify-content-between align-items-center">
                ...
                <span class="btn btn-primary my-2" data-bs-toggle="modal" data-bs-target="#userRegisterModal">Add User</span>
            </div>
            ...
            <DataTable :value="filteredRequests" stripedRows columnResizeMode="fit" removableSort paginator :rows="15"
                ...
                <Column header="Actions" style="width: 20%; text-align: center;">
                    <template #body="rowData">
                        ...
                        <button @click="deleteuser(rowData)" data-bs-toggle="modal" data-bs-target="#userDeleteModal"
                            class="btn">
                ...
                <div class="modal-dialog modal-dialog-centered">
                    <div class="modal-content">
                        ...
                        <div class="modal-body">
                            ...
                            <FloatLabel class="w-100 mt-4">
                                <InputText type="password" class="w-100" name="changePassword" v-model="userPassword" />
            ...
            <!-- user register Modal -->
            <div class="modal fade" id="userRegisterModal" tabindex="-1" aria-labelledby="userRegisterModal"
                aria-hidden="true">
                <div class="modal-dialog modal-dialog-centered">
                    <div class="modal-content">
                        <div class="modal-header">
                            <h5 class="modal-title" id="userRegisterModal">Create user</h5>
                            <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                        </div>
                        <div class="modal-body">
                            <FloatLabel class="w-100 mt-3">
                                <InputText class="w-100" v-model="userRegister.name" />
                                <label>Name</label>
                            </FloatLabel>
                            <FloatLabel class="w-100 mt-4">
                                <InputText class="w-100" v-model="userRegister.email" />
                                <label>Email</label>
                            </FloatLabel>
                            <FloatLabel class="w-100 mt-4">
                                <InputText type="password" class="w-100" name="changePassword"
                                    v-model="userRegister.password" />
                                <label>Password</label>
                            </FloatLabel>
                            <FloatLabel class="w-100 mt-4">
                                <InputText type="password" class="w-100" name="changePassword"
                                    v-model="userRegister.passwordC" />
                                <label>Confirm Password</label>
                            </FloatLabel>
                        </div>
                        <div class="modal-footer">
                            <button type="button" class="btn btn-secondary" data-bs-dismiss="modal">Close</button>
                            <button type="button" class="btn btn-primary" @click="updateUser">Create</button>
                        </div>
                    </div>
                </div>
            </div>
            ...
            <div class="modal fade" id="userDeleteModal" tabindex="-1" aria-labelledby="userDeleteModal"
                aria-hidden="true">
                <div class="modal-dialog modal-dialog-centered">
                    <div class="modal-content">
                        <div class="modal-header">
                            <h5 class="modal-title" id="userDeleteModal">Delete {{ userClicked[1] }}</h5>
                            <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                        </div>
                        <div class="modal-body">
                            This action is permanent. <br> Are you sure you want to delete {{ userClicked[1] }} ?
                        </div>
                        <div class="modal-footer">
                            <button type="button" class="btn btn-secondary" data-bs-dismiss="modal">Close</button>
                            <button type="button" class="btn btn-danger" @click="deletePermanentlyUser">Delete</button>
                        </div>
                    </div>
                </div>
            </div>
...
export default {
    ...
    methods: {
        ...
        deletePermanentlyUser() {
            ...
            axios.post(apiLink + "/api/deleteAccount", { id: this.userClicked[0] })
                .then(({ data }) => {
        ...
        updateUser() {
            axios.defaults.headers.common["Authorization"] =
                "Bearer " + $cookies.get('jwtoken');
            axios.post(apiLink + "/api/register", this.userRegister )
                .then(({ data }) => {
        ...
        createUser() {
            ...
            if (userRegister.password === userRegister.passwordC) {
